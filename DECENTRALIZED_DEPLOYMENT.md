# INGRVM: Decentralized & Homelab Deployment Guide 🌿🏗️
...
2. **Create a Deployment (SDL):** Use the generated `INGRVM/Core/lighthouse-compose.yml` but convert it to Stack Definition Language (SDL).
3. **Bid for Compute:** Providers will bid to host your node for a few cents a day in $AKT.
4. **Deploy:** Your INGRVM Lighthouse is now live on a decentralized server.
...
3. **Installation:**
   ```bash
   git clone https://github.com/INGRVM-Mesh/INGRVM-Core.git
   cd INGRVM-Core/INGRVM/Core
...
3. Run our pre-built stack:
   ```bash
   docker-compose -f INGRVM/Core/lighthouse-compose.yml up -d
   ```
...
---
*Drafted by The Architect | Powered by the INGRVM Mesh*
